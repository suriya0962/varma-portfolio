

const Myskills = () => {
  return (
    <div className=" bg-[#161616] md:h-[100vh] h-[300px] w-[100%]  ">
        <div className="md:flex md:gap-10 gap-5 " data-aos='slide-left' data-aos-delay='1000'>
     <div className="bg-[#161616] shadow-xl md:h-[60px] h-[20px] md:w-[25%] w-[200px] md:ml-[50px] ml-[20px] md:mt-[50px] mt-[10px] flex md:gap-4 gap-3" >
          
          <div >
        <img src='https://upload.wikimedia.org/wikipedia/commons/thumb/1/18/C_Programming_Language.svg/926px-C_Programming_Language.svg.png'
          className="md:h-[50px] h-[12px] md:bg-white bg-white md:w-[40px] w-[14px] md:mt-[5px] mt-[2px]"/>
          </div>
          <div className=" ">
          <h1 className="text-white md:mt-2   md:text-[25px] text-[13px] font-bold">C</h1>
         
       </div>
      </div>
      <div className="bg-[#161616] shadow-xl md:h-[60px] h-[20px] md:w-[25%] w-[200px] md:ml-[50px] ml-[20px] md:mt-[50px] mt-[10px] flex md:gap-4 gap-2" >
          
          <div>
        <img src='https://cdn-icons-png.flaticon.com/512/919/919827.png'
          className="md:h-[50px] h-[15px] bg-white md:w-[40px] w-[15px] md:mt-[5px] mt-[3px]"/>
          </div>
          <div className=" ">
          <h1 className="text-white md:mt-2 mt-1 md:text-[25px] text-[10px] font-bold">HTML </h1>
         
       </div>
      </div>
      <div className="bg-[#161616] shadow-xl md:h-[60px] h-[20px] md:w-[25%] w-[200px] md:ml-[50px] ml-[20px] md:mt-[50px] mt-[10px] flex md:gap-4 gap-2" >
          
          <div>
        <img src='https://cdn-icons-png.flaticon.com/512/732/732190.png'
          className="md:h-[50px] h-[15px] bg-white md:w-[40px] w-[15px] md:mt-[5px] mt-[3px]"/>
          </div>
          <div className=" ">
          <h1 className="text-white md:mt-2 md:text-[25px] text-[10px] font-bold mt-1">CSS</h1>
         
       </div>
      </div>
      </div>
      <div className="md:flex md:gap-10 gap-5" data-aos='slide-right' data-aos-delay='1000'>
      <div className="bg-[#161616] shadow-xl md:h-[60px] h-[20px] md:w-[25%] w-[200px] md:ml-[50px] ml-[20px] md:mt-[50px] mt-[10px] flex md:gap-4 gap-1" >
          
          <div >
        <img src='https://cdn-icons-png.flaticon.com/512/5968/5968292.png'
          className="md:h-[50px] h-[15px] bg-white md:w-[40px] w-[15px] md:mt-[5px] mt-[2px]"/>
          </div>
          <div className=" ">
          <h1 className="text-white md:mt-2 md:text-[25px] text-[12px] mt-[2px] font-bold">JavaScript</h1>
        
       </div>
      </div>
      <div className="bg-[#161616] shadow-xl md:h-[60px] h-[20px] md:w-[25%] w-[200px] md:ml-[50px] ml-[20px] md:mt-[50px] mt-[10px] flex md:gap-4 gap-2" >
          
          <div>
        <img src='https://cdn.iconscout.com/icon/free/png-256/free-react-1-282599.png?f=webp'
          className="md:h-[50px] h-[15px] bg-white md:w-[40px] w-[15px] md:mt-[5px] mt-[2px]"/>
          </div>
          <div className=" ">
          <h1 className="text-white md:mt-2 md:text-[25px] text-[12px] mt-[2px] font-bold">React</h1>
         
       </div>
      </div>
      <div className="bg-[#161616] shadow-xl md:h-[60px] h-[20px] md:w-[25%] w-[200px] md:ml-[50px] ml-[20px] md:mt-[50px] mt-[10px] flex md:gap-4 gap-2" >
          
          <div>
        <img src='https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS8rK-LE52N0d05lb7yCImZyEfwaTKb_KLj2A0IfKolsA&s'
          className="md:h-[50px] h-[15px] bg-white md:w-[40px] w-[15px] md:mt-[5px] mt-[0px]"/>
          </div>
          <div className=" ">
          <h1 className="text-white md:mt-2 md:text-[25px] text-[12px] mt-[2px] font-bold">Tailwind</h1>
         
       </div>
      </div>
      </div>
      <div className="md:flex md:gap-10 gap-5" data-aos='slide-left' data-aos-delay='1000'>
     <div className="bg-[#161616] shadow-xl md:h-[60px] h-[20px] md:w-[25%] w-[200px] md:ml-[50px] ml-[20px] md:mt-[50px] mt-[10px] flex md:gap-4 gap-2" >
          
          <div>
        <img src='https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png'
          className="md:h-[50px] h-[20px] bg-white md:w-[40px] w-[15px] md:mt-[5px] mt-[2px]"/>
          </div>
          <div className=" ">
          <h1 className="text-white mt-1 md:text-[25px] text-[12px] font-bold">Github</h1>
         
       </div>
      </div>
      <div className="bg-[#161616] shadow-xl md:h-[60px] h-[20px] md:w-[25%] w-[200px] md:ml-[50px] ml-[20px] md:mt-[50px] mt-[10px] flex md:gap-4 gap-2" >
          
          <div>
        <img src='https://cdn-icons-png.flaticon.com/512/5968/5968381.png'
          className="md:h-[50px] h-[20px] bg-white md:w-[40px] w-[20px] mt-[5px]"/>
          </div>
          <div className=" ">
          <h1 className="text-white mt-2 md:text-[25px] text-[12px] font-bold">Typescript</h1>
         
       </div>
      </div>
      <div className="bg-[#161616] shadow-xl md:h-[60px] h-[20px] md:w-[25%] w-[200px] md:ml-[50px] ml-[20px] md:mt-[50px] mt-[10px] flex md:gap-4 gap-2" >
          
          <div>
        <img src='data:image/jpeg;base64,'
          className="md:h-[50px] h-[20px] bg-white md:w-[40px] w-[20px] mt-[5px]"/>
          </div>
          <div className=" " >
          <h1 className="text-white md:mt-2 md:text-[25px] mt-[6px] text-[12px] font-bold">Bootstrap</h1>
         
       </div>
      </div>
      </div>
      <div className="flex justify-center md:mr-[50px] md:mt-[90px] mt-[35px]" data-aos='slide-right' data-aos-delay='1000'>
     <button className="flex justify-center text-white  md:text-[20px] text-[10px] hover:bg-white hover:text-black border-2  md:w-[120px] w-[80px] md:h-[40px] h-[20px] font-semibold">My Projects</button>
     </div>
     </div>
   
  )
}

export default Myskills